              ...
              <div class="panel panel-flat">
                ...
                <table class="table datatable-column-search-selects">
                  ...
                  <?php 	for($i=0; $i < count($country_list);++$i){?>
                    ...
                    <tr>
															<td><?php echo $country_list[$i]['id']; ?></td>
                              <td><?php echo $country_list[$i]['country']; ?></td>
                              <td><?php if($country_list[$i]['status']=='A'){echo "<span class='label label-success'>Active</span>";}else{echo "<span class='label label-danger'>Inactive</span>";} ?></td>
                              <td hidden=""></td>
                              <td hidden=""></td>

                      <td class="text-center">
                        <?php if($this->session->userdata('UserGroup') == 'Admin' || $this->session->UserGroup == "Doctor"){ ?>
                        <a class="btn border-info text-info-600 btn-flat btn-icon btn-rounded" href="<?php echo base_url();?>index.php/country/edit_country/<?php echo $country_list[$i]['id'];?>?page=drug"><i class="icon-pencil7"></i></a>
                      <?php } ?>
