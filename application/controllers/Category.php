...
//Save Drugs
         ...
         echo json_encode($this->form_validation->error_array());
					...
					if($this->input->post('status') == 'on'){
 					...
 					$data =  array('category' => $this->input->post('category'),							
 							...
 							echo json_encode(['success' => ['msg' => "Category created successfully"]]);

 					$this->CategoryModel->save_category($data);
	...
	public function edit_category($id){
		...
		$data["edit_category"] = $this->db->get_where('category', array('id' => $id))->result_array();
		if(empty($data['edit_category'])){
			$data["error"] = " category not found";
			$this->load->vars($data);
			$this->load->view('errors/error_message');
			return;
			...
			if($this->input->post('status') == 'on'){
				...
				$status = 'I';
					...
					$data =  array('category' => $this->input->post('category'),							
							...
							http_response_code(200);
							echo json_encode(['success' => ['msg' => "Category updated successfully"]]);
