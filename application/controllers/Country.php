...
//Save Drugs
         ...
         echo json_encode($this->form_validation->error_array());
					...
					if($this->input->post('status') == 'on'){
 					...
 					$data =  array('country' => $this->input->post('country'),								
 							...
 							echo json_encode(['success' => ['msg' => "Country created successfully"]]);

 					$this->CountryModel->save_country($data);
	...
	public function view_country(){
		...
		$this->load->vars($data);
		$this->load->view('country/view_country');
	}
	...
	public function edit_country($id){
		...
		if(!isset($id) ||(!is_numeric($id) )){
			$data["error"] = "countyr not found";
			$this->load->vars($data);
			$this->load->view('errors/error_message');
			return;
		}

		$data["edit_country"] = $this->db->get_where('country', array('id' => $id))->result_array();
		if(empty($data['edit_country'])){
		...
		$this->load->vars($data);
		$this->load->view('country/edit_country');
	}
			...
			if($this->input->post('status') == 'on'){
				...
				$status = 'I';
					...
					$data =  array('country' => $this->input->post('country'),								
							...
							http_response_code(200);
							echo json_encode(['success' => ['msg' => "Country updated successfully"]]);

					$this->CountryModel->update_country($id,$data);
