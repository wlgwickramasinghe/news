...
//Save Drugs
	 public function add_video(){
		 $this->load->view('video/add_video');
	 }

	 public function save_video(){

 		$this->form_validation->set_rules('video', 'Video Name', 'required');
         ...
         echo json_encode($this->form_validation->error_array());
					...
					if($this->input->post('status') == 'on'){
 					...
 					$data =  array('video_name' => $this->input->post('video'),	
 							...
 							echo json_encode(['success' => ['msg' => "video created successfully"]]);

 					$this->VideoModel->save_video($data);
	...
	public function view_video(){
		$data["video_list"] = $this->db->get('video')->result_array();	
		$this->load->vars($data);
	...
	public function edit_video($id){
		...
		$data["edit_video"] = $this->db->get_where('video', array('id' => $id))->result_array();
		if(empty($data['edit_video'])){
			$data["error"] = " video not found";
			$this->load->vars($data);
			$this->load->view('errors/error_message');
			return;
		...
		$this->load->vars($data);
		$this->load->view('video/edit_video');
	}
			...
			if($this->input->post('status') == 'on'){
				...
				$status = 'I';
					...
					$data =  array('video_name' => $this->input->post('video'),	
							...
							http_response_code(200);
							echo json_encode(['success' => ['msg' => "video updated successfully"]]);

					$this->VideoModel->update_video($id,$data);
